...
include_once '../connection.php';
        ...
        if (isset($_POST['update'])) {
            ...
            $pass1 = mysqli_real_escape_string($conn, SHA1($_POST['pass1']));
                ...
                $select = mysqli_query($conn, "SELECT * FROM tbl_municipal_tourism WHERE user_id = '$id'");

                if (mysqli_num_rows($select) <= 0) {
                    $_SESSION['status'] = 'Sorry Failed to Update Your Account';
                    header("location: ../../change_pass.php");
                }else {
                        $sql = mysqli_query($conn, "UPDATE tbl_municipal_tourism
                        SET mt_username = '$uname', mt_password = '$pass1'
                        WHERE user_id = '$id'");
