...
if (isset($_POST['qrcode'])) {
    ...
    WHERE tracker = '$code' AND tbl_ts_manager.tsm_id = '$tsm' LIMIT 1");

    if (mysqli_num_rows($scan) >= 1) {
            $row = mysqli_fetch_assoc($scan);
        if ($row['status'] === "Pending") {
            $tourist_id = $row['pktourist_id'];
            $query_pending = mysqli_query($conn, "UPDATE tbl_tourist_info SET status = 'Arrived' WHERE pktourist_id ='$tourist_id' AND tsm_id = '$tsm'  ");
            ...
            $tourist_id = $row['pktourist_id'];
            $query_arrived = mysqli_query($conn, "UPDATE tbl_tourist_info SET status = 'Out' WHERE pktourist_id ='$tourist_id' AND tsm_id = '$tsm'  ");
